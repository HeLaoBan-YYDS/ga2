...
export async function SiteFooter({ locale }: { locale: string }) {
  ...
  return <footer className="mt-16 border-t border-border bg-card/30"><div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8"><div className="mb-10 rounded-2xl border border-border bg-muted/40 p-5"><div className="font-bold text-foreground">{t("aboutTitle")}</div><p className="mt-1 text-sm text-muted-foreground">{t("description")}</p><Link href="https://www.roblox.com/games/97598239454123/Grow-a-Garden-2" className="mt-3 inline-flex items-center gap-2 text-sm font-semibold text-[hsl(var(--nav-theme))]">{t("playGame")} <ExternalLink className="h-4 w-4" /></Link></div><p className="mb-8 text-xs font-bold uppercase tracking-[0.2em] text-muted-foreground">{site("legalNotice")}</p><div className="grid gap-8 md:grid-cols-4"><div className="md:col-span-2"><h3 className="font-bold text-foreground">{t("aboutTitle")}</h3><p className="mt-3 max-w-xl text-sm leading-7 text-muted-foreground">{t("about")}</p></div><FooterList title={t("quickLinks")} links={[[t("playGame"), "https://www.roblox.com/games/97598239454123/Grow-a-Garden-2"], [t("officialDiscord"), "https://discord.gg/H8qPF3QnMF"], [t("officialYoutube"), "https://www.youtube.com/watch?v=a3TCmxcT2QI"], [t("officialX"), "https://x.com/GrowaGardenRblx"]]} /><FooterList title={t("guides")} links={[[t("beginnerGuide"), "/guide"], [t("seedGuides"), "/crops"], [t("defenseGuides"), "/guide"], [t("petGuides"), "/pets"], [t("privacyPolicy"), "/privacy-policy"], [t("termsOfService"), "/terms-of-service"]]} /></div><p className="mt-10 border-t border-border pt-6 text-xs text-muted-foreground">{t("copyright")}</p></div></footer>;
}

function FooterList({ title, links }: { title: string; links: string[][] }) { return <div><h4 className="font-semibold text-foreground">{title}</h4><ul className="mt-3 space-y-2 text-sm text-muted-foreground">{links.map(([label, href]) => <li key={`${label}-${href}`}><Link className="hover:text-foreground" href={href}>{label}</Link></li>)}</ul></div>; }
...
function openTrailerDialog(videoId: string) {
  ...
  frame.src = `https://www.youtube.com/embed/${videoId}?autoplay=1&origin=${encodeURIComponent(window.location.origin)}`;
  dialog.showModal();
